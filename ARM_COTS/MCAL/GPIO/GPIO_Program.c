...
void GPIO_voidSetPortMode(GPIO_PORT_e portID, GPIO_MODE_e portMode)
{
    ...
    {
        // Variable to store the constructed mode value
        u32 modeValue = 0;

        // Iterate through all pins (16 pins) and set the modeValue accordingly
        for (int i = 0; i < 16; i++)
        {
...
void GPIO_voidSetPortOutputMode(GPIO_PORT_e portID, GPIO_OUTPUT_MODE_e portOutputMode)
{
    ...
    if ((portID <= GPIO_PORTC))
    {
        ...
        switch (portID)
        {
            // If port is A
            case GPIO_PORTA:
                GPIOA_OTYPER = portOutputMode;
                break;

            // If port is B
            case GPIO_PORTB:
                GPIOB_OTYPER = portOutputMode;
                break;

            // If port is C
            case GPIO_PORTC:
                GPIOC_OTYPER = portOutputMode;
                break;
        }
    }
    else
